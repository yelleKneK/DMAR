...
normality. Two formulas are available: the Olkin-Finn (1995) asymptotic
variance on the raw \\r\\ scale (the default), and the Hotelling (1953)
variance of the Fisher *z*-transformation, which is the appropriate
quantity for constructing a confidence interval by transformation and
back-transformation.
...
- fisher_z:
  ...
  Logical. If `FALSE` (the default), the function returns the
  Olkin-Finn (1995) asymptotic variance on the raw scale. If `TRUE`, it
  returns the variance of the Fisher *z*-transformation \\z =
  \mathrm{arctanh}(r)\\ under the Hotelling (1953) result.
...
variance (Olkin & Finn, 1995, equation 11): \$\$\mathrm{Var}(\hat r\_{XY
\cdot Z}) \\\approx\\ \frac{(1 - \rho^2\_{XY \cdot Z})^2}{n - J -
1},\$\$ a direct generalization of the classical asymptotic variance
\\(1 - \rho^2)^2 / (n - 1)\\ of the simple Pearson correlation (Fisher,
1915) with the degrees of freedom reduced by the number of partialled
variables. The function evaluates this with \\\hat r\\ substituted for
\\\rho\\.

**Fisher's *z*-transformation.** Fisher (1915, 1921) showed that for a
Pearson correlation, the transformation \\z =
\tfrac{1}{2}\log\\(1+r)/(1-r)\\ = \mathrm{arctanh}(r)\\ is approximately
normal with variance \\1/(n - 3)\\. Hotelling (1953) extended this
result to partial correlations: the Fisher *z*-transformation of \\\hat
r\_{XY \cdot Z_1 \cdots Z_J}\\ is approximately normal with variance
\\1/(n - J - 3)\\. This is the standard ingredient for constructing a
confidence interval on \\\rho\_{XY \cdot Z}\\ by transforming, building
a Wald interval on \\z\\, and back-transforming with \\\tanh\\.
...
# Variance of the Fisher z-transformation (Hotelling 1953) for the
