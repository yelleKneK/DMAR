...
2010). Uses the Fisher-\\z\\ transformation throughout, so the test is
exact under bivariate normality and approximately so for other
continuous distributions. Equivalence is declared when the 100(1 -
2\\\alpha\\)% Fisher- \\z\\ CI on \\\rho\\ lies entirely inside the
equivalence region.
...
statistics on the Fisher-\\z\\ scale, their *p*-values, the joint TOST
*p*-value, the 100(1 - 2\\\alpha\\)% CI on \\\rho\\, the equivalence
bounds, and a binary equivalence flag.

## Details

**Fisher-\\z\\ transformation.** \$\$z = \tfrac{1}{2}
\log\left(\frac{1 + r}{1 - r}\right), \quad \mathrm{Var}(z) =
\frac{1}{n - 3}.\$\$ The TOST is run on the \\z\\ scale:
...
tost_r(r = 0.05, n = 200, rho_upper = 0.10)
#>  term         value 
#>  r            0.05  
#>  z_lower_test 2.11  
#>  z_upper_test -0.706
