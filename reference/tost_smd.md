...
statistics (`t_lower`, `t_upper`), their two one-sided *p*-values
(`p_lower`, `p_upper`), the joint TOST *p*-value (the larger of the
two), the 100(1 - 2\\\alpha\\)% CI on \\\delta\\, the equivalence
bounds, and a binary decision flag (`equivalent`: 1 = equivalent, 0 =
not).
...
tost_smd(x = x, y = y, delta_upper = 0.4)
#>  term        value 
#>  smd         0.23  
#>  t_lower     3.14  
#>  t_upper     -0.845
...
tost_smd(smd = 0.05, n_1 = 40, n_2 = 40, delta_upper = 0.3)
#>  term        value 
#>  smd         0.05  
#>  t_lower     1.57  
#>  t_upper     -1.12 
...
tost_smd(smd = 0.10, n_1 = 80, n_2 = 80,
