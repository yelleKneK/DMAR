...
indirect effect \\\hat a \hat b\\, and forms a CI via either the Sobel
first-order approximation or a Monte Carlo CI on the same scale used by
...
- method:
  ...
  One of `"sobel"` (default) or `"monte_carlo"`; the CI method used in
  the simulation and forwarded to the planner.
