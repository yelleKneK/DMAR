...
# ci_nc_F(): ci_snr(), ci_srsnr(), ci_pvaf(), ci_R2() with fixed
...
#
# * When ci_nc_F() sets the lower noncentrality limit to 0 because
...
#   interval the user asked for, not ci_nc_F()'s internals.
#   .ci_nc_F_for() muffles the clamp warning (condition class
#   "dmar_nc_F_clamp") and re-signals it with the calling function's own
...
.ci_nc_F_for <- function(caller, quantity, F_value, df_1, df_2,
                                 conf_level = NULL, alpha_lower = NULL,
                                 alpha_upper = NULL, ...) {
  fmt <- function(x) if (is.null(x)) "NULL" else format(x)
  withCallingHandlers(
    tryCatch(
      ci_nc_F(F_value = F_value, conf_level = conf_level,
                      df_1 = df_1, df_2 = df_2,
    ...
    dmar_nc_F_clamp = function(w) {
      warning(warningCondition(sprintf(
        ...
        quantity), class = "dmar_nc_F_clamp"))
