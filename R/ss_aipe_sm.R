...
#'
#' @seealso \code{\link{ci_nc_t}}, \code{\link{ci_sm}}
#'
#' @examples
...
ss_aipe_sm <- function(sm, width, conf_level = .95, assurance = NULL, ...) {
  ...
  # The iterative search calls ci_nc_t() repeatedly with intermediate
  ...
  # which pt()/qt() lose accuracy, and ci_nc_t() signals this with a
  # warning. Surfacing that warning once per iteration produces dozens of
  # identical messages (see ?ci_nc_t). We muffle the per-iteration
  ...
  if (.top_level) {
    ...
    on.exit({
      if (.ncp_env$count > 0L) {
        warning(sprintf(
          "During the iterative sample size search, ci_nc_t() reported a noncentrality parameter exceeding 37.62 in magnitude in %d intermediate evaluations, the limit at which R's pt()/qt() can return accurate noncentral t probabilities. The returned sample size may be affected; see ?ci_nc_t.",
          .ncp_env$count
        ), call. = FALSE)
      }
    }, add = TRUE)
  ...
  if (is.null(assurance)) { # Initial starting value for n using the z distribution.
    ...
    lambda_limits_0 <- ci_nc_t(ncp = lambda_0, df = n - 1, conf_level = 1 - alpha)
    ...
    while (Diff_width_Full > 0) {
      n <- n + 1
      lambda <- sm * sqrt(n)
      lambda_limits <- ci_nc_t(ncp = lambda, df = n - 1, conf_level = 1 - alpha)
