# Simulate longitudinal data from the four parameter Gompertz growth curve.
#' Simulate Data From a Gompertz Change (Growth) Model
#'
#' Generates longitudinal data from a random-coefficients Gompertz change
#' model: each unit (a person, an animal, a tree) follows an S-shaped curve that, unlike the
#' logistic, is not symmetric about its point of inflection, the
#' parameters vary randomly across units, and each measurement adds
#' level-one error. The deterministic part of the four parameter Gompertz
#' curve is
#' \deqn{\mu(t) = \alpha \exp\left(-\exp(-\gamma (t - \beta))\right) + \zeta,}
#' the parameterization of Kelley (2005, 2008), which generalizes the
#' three parameter Gompertz of the literature (Winsor, 1932; Ratkowsky,
#' 1983) by adding \eqn{\zeta} so the lower asymptote is itself a
...
#'       Gompertz inflection is early and asymmetric: it occurs where
#'       the curve passes through \eqn{\alpha / e + \zeta}, about
#'       36.8\% of the total change, so growth accelerates briefly and
#'       decelerates over a long approach to the ceiling.}
...
#'       The intercept is
#'       \eqn{\phi = \alpha \exp(-\exp(\gamma \beta)) + \zeta}.}
#'   }
...
#' learning and development, are natural Gompertz candidates. The
#' Gompertz is the \eqn{\delta \rightarrow 0} limit of the Richards
#' curve (\code{\link{simulate_longitudinal_richards}}), which frees the
#' inflection entirely (Kelley, 2005, 2008).
#'
#' @references
...
#'   \emph{Proceedings of the National Academy of Sciences, 18}(1),
#'   1--8.
#'
#' @seealso \code{\link{simulate_longitudinal_logistic}} for the
#'   symmetric sibling; \code{\link{simulate_longitudinal_richards}} for
#'   the family that subsumes both;
...
#'
#' @export
simulate_longitudinal_gompertz <- function(n,
                                           target_times = NULL,
                                           fixed_parameters,
                                           time_range = NULL,
                                           occasions = NULL,
