...
#' @param beta_j The regression coefficient for the \emph{j}th predictor variable (i.e., the predictor of interest)
...
#' @param assurance Degree of certainty that the obtained confidence interval will be sufficiently narrow, which yields an approximate sample size to be verified with function \code{ss_aipe_reg_coef_sensitivity} to determine if it is appropriate
#'
#' @details
...
#'
#' @note
#' This function calls upon \code{\link{ss_aipe_reg_coef}} in DMAR but has a different naming scheme.
#' See \code{\link{ss_aipe_reg_coef}} for more details.
#'
...
#'
#' @seealso
#' \code{\link{ss_aipe_reg_coef_sensitivity}}, \code{\link{ci_nc_t}}, \code{\link{ss_aipe_reg_coef}}, \code{\link{ss_aipe_rc}}
#'
#' @examples
...
ss_aipe_src <- function(rho2_Y_X = NULL, Rho2_j_X_without_j = NULL, p = NULL,
                        beta_j = NULL, width, which_width = "Full", sigma_Y = 1, sigma_X_j = 1, rho_XX = NULL,
                        rho_YX = NULL, which_predictor = NULL, alpha_lower = NULL, alpha_upper = NULL,
                        conf_level = .95, assurance = NULL) {
  return(ss_aipe_reg_coef(
    rho2_Y_X = rho2_Y_X, rho2_j_X_without_j = Rho2_j_X_without_j, p = p, b_j = beta_j,
    width = width, which_width = which_width, sigma_Y = sigma_Y, sigma_X = sigma_X_j,
    rho_XX = rho_XX, rho_YX = rho_YX, which_predictor = which_predictor, noncentral = TRUE,
    alpha_lower = alpha_lower, alpha_upper = alpha_upper, conf_level = conf_level,
    assurance = assurance
  ))
}
