---
title: "Heterogeneity of Regression in the Pygmalion Data"
author: "Ken Kelley"
...
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Heterogeneity of Regression in the Pygmalion Data}
...
`vignette("teacher_expectancy")`. The two are kept apart on purpose:
a single study and the accumulated evidence about its claim are
importantly different things, and DMAR treats them as such.
...
The same numbers ship with the book's data companion package
(**AMCP**) as `chapter_9_exercise_15`; here the experimental condition
is a labeled factor and the columns use DMAR's descriptive names. The
data set is the running example for **analysis of covariance (ANCOVA)
with heterogeneity of regression** in Maxwell, Delaney, and Kelley,
*Designing Experiments and Analyzing Data: A Model Comparison
Perspective* (Chapter 9). Thorndike's (1968) review questioned the
reliability of the underlying test (the Tests of General Ability, TOGA)
at the extremes of the score range for the youngest children, which is
part of why these data reward careful analysis.

## The Homogeneity-of-Regression Assumption

A standard ANCOVA uses the pretest (`iq_pre`) as a covariate and
assumes that the regression of the outcome on the covariate has the
**same slope in every group**. We can look at that assumption directly
by fitting separate slopes and overlaying them.
...
The control slope is `r round(coef(fit_het)["iq_pre"], 3)`; adding the
interaction gives a steeper bloomer slope of
`r round(coef(fit_het)["iq_pre"] + coef(fit_het)["iq_pre:treatmentBloomer"], 3)`.
...
`DMAR::ancova()` returns the adjusted (covariate-corrected) cell means,
the omnibus *F* for the treatment effect, partial `eta^2` and partial
`omega^2` with noncentral *F* confidence intervals, and the
homogeneity-of-regression test, all in one tidy data frame.
...
common-slope model. When the slopes genuinely differ, a single
...
treatment effect adjusting for pretest IQ,
*F*(`r res$value[res$term == "df_1"]`, `r res$value[res$term == "df_2"]`) =
`r round(res$value[res$term == "F_value"], 2)`,
*p* = `r format_p(res$value[res$term == "p_value"])`, with partial
$\eta^2$ = `r round(res$value[res$term == "eta_squared_partial"], 3)`,
95% CI [`r round(res$value[res$term == "eta_squared_partial_lower"], 3)`,
`r round(res$value[res$term == "eta_squared_partial_upper"], 3)`].
...
roughly `r round(effect_at(x_at["low"]), 1)` points at one SD below the
mean to about `r round(effect_at(x_at["high"]), 1)` points at one SD
above it. Evaluating, testing, and *planning for* a treatment effect at
chosen covariate values, rather than only at the grand mean, is exactly
the problem treated by the variance of the estimated treatment effect
methodology of Li, McLouth, and Delaney (2020), implemented natively
as `var_ete()`.
...
disappears once they do. See `vignette("teacher_expectancy")` for that
meta-analysis reproduced with `combine_p()`, `meta_contrast()`, and
`meta_smd()`.
...
(4th ed.). Routledge. (Heterogeneity-of-regression ANCOVA, Chapter 9.)
