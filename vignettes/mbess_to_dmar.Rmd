...
| `MBESS::conf.limits.nct(...)`              | `DMAR::ci_nct(...)`               | Noncentral *t*; `t.value -> t_value`.                                 |
| `MBESS::conf.limits.ncf(...)`              | `DMAR::ci_ncf(...)`               | Noncentral $F$.                                                       |
...
- A package-wide `term`/`value` `data.frame` schema; broom-style
  ...
  Carlo function, with explicit save/restore of
  `.Random.seed` when the user supplies a seed.
